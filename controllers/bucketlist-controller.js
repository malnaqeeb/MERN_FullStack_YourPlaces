...
const addToBucketList = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new HttpError("Something went wrong, could not find a user for the provided id.", 500),
    );
  }
  ...
  if (!isUnique) {
    const error = new Error("You can not add the place with provided id", 401);
    return next(error);
  }
  ...
  if (placeForBucket.creator != req.userData.userId && isUnique) {
    ...
    const error = new Error("You can not add your own places to you bucket list", 401);
    return next(error);
  }
  res.json({
